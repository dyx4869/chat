...
// P操作：等待信号量
sem_wait(&sem);   // 阻塞版本
sem_trywait(&sem); // 非阻塞版本，立即返回
sem_timedwait(&sem, &abs_timeout); // 限时等待
...
void addfriend(int cliendfd, string com)
{
 ...
 unordered_map<string, string> commandMap = {{"help", "显示所有命令,格式help"},
     {"chat", "私聊,格式chat:friendid:message"}, {"addfriend", "加好友,格式addfriend:friendid"},
     {"creategroup", "建群,格式creategroup:groupname:groupdesc"}, {"joingroup",
 "加群,格式joingroup:groupid"},
...
void doregresponse(json& response)
{
    if (response["errno"].get<int>() == 0) {
        cout << "id : " << response["id"].get<int>() << endl;
